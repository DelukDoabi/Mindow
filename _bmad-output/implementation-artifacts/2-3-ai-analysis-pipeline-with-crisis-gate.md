...
- [ ] **Task 1 — `weight.assigned` domain event + decoder + registration** (AC: #4, #6)
...
- [ ] **Task 3 — `AiClient` (client→Edge-Function seam)** (AC: #1, #2)
  ...
  - [ ] Create `lib/core/ai/ai_failure.dart`: a sealed `AiFailure` taxonomy (e.g., `AiNetworkFailure`, `AiTimeoutFailure`, `AiMalformedResponseFailure`) per the architecture `core/error` convention. `AiClient` throws/returns these on non-crisis errors so the orchestrator (Task 4) can drive fallback + retry. Do NOT leak `FunctionException` upward.
  - [ ] Expose `@riverpod AiClient aiClient(Ref ref)` reading `supabaseClientProvider`.

- [ ] **Task 4 — Analysis orchestration (consent → AI → event/fallback/crisis)** (AC: #2, #3, #5)
  - [ ] Add an analysis flow to the brain_dump feature (extend `BrainDumpRepository` or add a thin `AnalysisService` in `lib/features/brain_dump/` — keep it consistent with the repository pattern). Signature e.g. `Future<AnalysisOutcome> analyzePreoccupation({required String id, required String content})`.
  ...
  - [ ] Reuse the existing injected `Uuid` and `Clock` already on `BrainDumpRepository` for deterministic tests. Define the fallback constants in one place (e.g., `lib/features/brain_dump/domain/analysis_constants.dart`) — `kFallbackWeightKg`, `kFallbackEffortScore`, `kFallbackDurationMinutes`, `kFallbackCategory = 'Autre'`. See Open Questions for the exact values to confirm.
  - [ ] Trigger point: after a successful `capturePreoccupation` (Story 2.2), fire analysis without awaiting it on the UI path (fire-and-forget / `unawaited`) so capture stays instant (AC#3). After analysis enqueues an event, `ref.invalidate(openPreoccupationsProvider)` so the list refreshes pending → weighed. Also expose a way to (re)analyze still-pending items on app resume / next launch (so offline-captured items get weighed when back online) — a simple `analyzePendingPreoccupations()` that iterates pending items is sufficient for this story.

- [ ] **Task 5 — Bounded retry (cost guardrail)** (AC: #5, NFR-11)
  - [ ] Implement bounded retry for failed analyses: max `kMaxAnalysisRetries` attempts with backoff. Keep it simple and offline-tolerant — an in-memory/Hive-tracked attempt counter per Preoccupation id is acceptable for MVP; do NOT busy-loop. A retry that succeeds enqueues a fresh `weight.assigned` (latest wins in the projection). Document that durable cross-launch retry scheduling and per-user AI budget enforcement beyond the attempt cap are deferred (see Scope Boundaries).
  - [ ] Ensure no double-spend: the same Preoccupation must not trigger overlapping in-flight analyses (guard with an in-flight set keyed by id).

- [ ] **Task 6 — Crisis UX + support resources** (AC: #2)
  - [ ] When the orchestrator returns `AnalysisOutcome.crisis`, surface a calm, warm, no-judgment support view (dialog or dedicated screen under `lib/features/brain_dump/presentation/`). It shows compassionate copy + tappable support resources (phone / URL). It must NOT show a weight, category, or any gamification element.
  - [ ] For MVP, source the crisis resources client-side as a localized constant list (FR + EN) — e.g. `lib/features/brain_dump/domain/crisis_resources.dart` keyed by locale — rather than a backend table (simpler, vetted at build time). The Edge Function only needs to return `isCrisis: true`; the client renders the locale-appropriate resources. Flag the exact resource list for product/legal vetting (Open Questions).
  - [ ] The captured Preoccupation that triggered crisis detection: for MVP keep it as a pending item with no weight (it was already captured in 2.2) — do NOT auto-delete (delete is 2.4). Confirm this stance in Open Questions.

- [ ] **Task 7 — `ai-analyze` Supabase Edge Function** (AC: #1, #2)
  - [ ] Create `supabase/functions/ai-analyze/index.ts` (Deno/TypeScript), mirroring the structure of the existing `account-export`/`account-delete` functions (shared `cors.ts`, auth via JWT, env-based secrets). Read `OPENAI_API_KEY` from the Edge env (never client). Accept `{ content, language }`.
  - [ ] **Crisis-gate FIRST:** before any weighing, run the synchronous crisis classifier on `content`. On detection, return `{ isCrisis: true }` (HTTP 200) and STOP — no weighing, no Free/Premium branch (architecture: crisis-gate runs before any entitlement branch; safety is not a Premium feature). See Open Questions on the classifier mechanism (dedicated prompt vs. rule pre-filter + LLM).
  - [ ] **Weighing:** if not crisis, call GPT-4o Mini with a constrained prompt that returns strict JSON `{ category, mental_weight_kg, effort_score, estimated_duration_minutes }` where `category` ∈ the fixed nine. Stamp `weight_model_version` server-side (a constant identifying the prompt+model, e.g. `'gpt-4o-mini-2026-06'`). Validate/parse the model output defensively; on model error return a non-2xx or an error body so the client drives its fallback (AC#5).
  - [ ] Deployment is via the existing GitHub Actions workflow `.github/workflows/deploy-edge-functions.yml` (local `supabase` CLI deploy is blocked on the corporate network — see repo notes); add `ai-analyze` to the deploy job. Provision `OPENAI_API_KEY` as a Supabase secret. Local Dart tests must NOT call the real function — use a fake `AiClient`.

- [ ] **Task 8 — Localization (French = source of truth)** (AC: #2, #6)
  - [ ] Add ARB keys to `assets/l10n/app_fr.arb` then mirror in `assets/l10n/app_en.arb` for: the nine Category display labels, the pending/analysing label (if distinct from 2.2's), crisis-support title + body + resource labels, and any fallback/no-consent soft message. Provide `@key` descriptions. Tone: warm, tutoiement, no guilt, no clinical claims (NFR-8).
  - [ ] Run `flutter gen-l10n` and reference via `AppLocalizations.of(context)`. Category labels: store the canonical category as the stable English/French token on the event (`'Santé'` etc. per PRD) but render via l10n so UI text stays localizable — confirm the canonical stored value vs. display mapping in Dev Notes.

- [ ] **Task 9 — Tests** (AC: all)
  - [ ] `test/features/brain_dump/domain/weight_assigned_event_test.dart`: `eventType == 'weight.assigned'`, `toJson` snake_case round-trip, decode through `decodeWeightAssigned`, and decode via the `DomainEventRegistry`.
  - [ ] `test/features/brain_dump/brain_dump_repository_test.dart` (extend): folding a `weight.assigned` onto a captured aggregate sets weight/category and clears pending; a fallback event leaves pending=false with `category == 'Autre'`; latest weight event wins; an orphan `weight.assigned` (no prior capture) is ignored.
  - [ ] Analysis-orchestration tests with a FAKE `AiClient`: consent denied → no event, stays pending; success → one `weight.assigned` enqueued with mapped fields; crisis → no event + crisis outcome; failure → fallback event enqueued + retry scheduled (bounded, no infinite loop); in-flight guard prevents double analysis.
  - [ ] (If feasible) a Deno test for `ai-analyze` asserting crisis-gate returns `{isCrisis:true}` before weighing and that a normal input yields the constrained JSON shape. Otherwise document manual verification post-deploy.
  - [ ] Validate: `dart run build_runner build` (no `--delete-conflicting-outputs`), `flutter gen-l10n`, `flutter analyze` (0 issues), `dart format lib test`, and confirm the `event_contract_parity_test` still passes. Run `flutter test`.
...
### Open Questions (flag with architect/product before/at dev kickoff)

1. **Mental Weight → kg scale/calibration (architecture Q1, FR-6).** What is the valid kg range and how is the GPT prompt anchored so the number is comparable across users/time? Provide the prompt's scale guidance (e.g., 1–50 kg) and confirm whether kg is a displayed truth or internal signal.
2. **Fallback floor + neutral constants.** Exact values for `kFallbackWeightKg`, `kFallbackEffortScore`, `kFallbackDurationMinutes`. The floor must be low/neutral (so a heavier real value never feels like a punishing jump) yet distinct from `null`. Default proposal pending confirmation: weight 3 kg, effort 3 (of 5), duration 30 min.
3. **Effort Score scale.** Range + meaning (proposal: integer 1–5). Confirm so the prompt and UI agree.
4. **Frozen-weight vs. fallback-then-retry reconciliation.** When a retry improves a fallback weight, does "latest event wins" risk an upward jump that violates "never worsens silently"? Confirm the projection rule (latest wins vs. floor-clamp vs. take-min/take-max). MVP default: latest wins, documented as revisit-in-2.5.
5. **Crisis classifier mechanism + latency target.** Dedicated fast prompt, a rules pre-filter + LLM, or a separate model? What is the acceptable synchronous latency budget for the gate (architecture Q7/§8)? Confirm the detection taxonomy/thresholds (self-harm, suicide, abuse, …) with a human owner (architecture principle 6 — "product/ethical decision with a human owner").
6. **Crisis resources sourcing + vetting.** MVP stores a localized constant list client-side. Confirm the actual vetted FR/EN resources (orgs, numbers, URLs) and whether they should later move to a deployable backend config.
7. **Crisis item disposition.** On crisis, the already-captured Preoccupation stays as a pending item (no auto-delete in 2.3). Confirm this is acceptable vs. offering the user to remove it.
8. **Event emission authority.** MVP emits `weight.assigned` from the CLIENT after the Edge response (client UUID, offline-tolerant, dedup by event_id), synced later. Confirm vs. server-emitted-then-reconciled (affects 2.x reconcile design).
